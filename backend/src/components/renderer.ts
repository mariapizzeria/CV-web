import {EducationApiResponse} from "../api/payload";
    ...
    public createEducationItem(edu: EducationApiResponse) {
        ...
        <div class="timeline-item flex flex-col md:flex-row items-center relative">
                    <div class="timeline-content w-full md:w-1/2 text-right pr-8 md:pr-12 mb-4 md:mb-0">
                        <h3 class="caveat text-2xl md:text-3xl text-accent">${edu.duration}</h3>
                        <span class="text-gray-600">${edu.college}</span>
                        <p class="font-bold text-xl md:text-2xl text-accent mt-2">${edu.course}</p>
                    </div>
